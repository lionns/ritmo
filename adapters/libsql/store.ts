...
import type { InValue as SQLInputValue } from "@libsql/client/web";
import { RemoteDatabase } from "./database.ts";
...
export class LibsqlStore implements Store, AuthStore {
  readonly #database: RemoteDatabase;

  constructor(database: RemoteDatabase) {
    this.#database = database;
  }

  async createCredential(c: Credential): Promise<void> {
    await this.#database.prepare(`INSERT INTO credentials
  ...
  async getCredential(credentialId: string): Promise<Credential | null> {
    const row = await this.#database.prepare(`SELECT id, owner_id AS ownerId, label,
  ...
  async listCredentials(ownerId: string): Promise<Credential[]> {
    const rows = await this.#database.prepare(`SELECT id, owner_id AS ownerId, label,
      credential_id AS credentialId, public_key AS publicKey, sign_count AS signCount,
  ...
  async deleteCredential(id: string, ownerId: string): Promise<void> {
    await this.#database.prepare("DELETE FROM credentials WHERE id = ? AND owner_id = ?").run(id, ownerId);
  }
  async advanceCredential(id: string, previousCount: number, count: number, usedAt: string): Promise<boolean> {
    const result = await this.#database.prepare(`UPDATE credentials SET sign_count = ?, last_used_at = ?
  ...
  async saveChallenge(id: string, ownerId: string, purpose: string, expiresAt: number): Promise<void> {
    await this.#database.prepare("DELETE FROM auth_challenges WHERE expires_at <= ?").run(expiresAt - 300000);
    await this.#database.prepare("INSERT INTO auth_challenges VALUES (?, ?, ?, ?)").run(id, ownerId, purpose, expiresAt);
  }
  async consumeChallenge(id: string, ownerId: string, purpose: string, now: number): Promise<boolean> {
    const result = await this.#database.prepare(`DELETE FROM auth_challenges
  ...
  async allowAuthAttempt(ownerId: string, window: number): Promise<boolean> {
    const row = await this.#database.prepare(`INSERT INTO auth_attempts (owner_id, window, attempts) VALUES (?, ?, 1)
  ...
  async getOwner(id: string): Promise<Owner | null> {
    const row = await this.#database.prepare("SELECT * FROM owners WHERE id = ?").get(id);
    return row === undefined ? null : toOwner(row as unknown as OwnerRow);
  }

  async getOnlyOwner(): Promise<Owner | null> {
    const rows = await this.#database.prepare("SELECT * FROM owners ORDER BY id LIMIT 2").all();
  ...
  async getArea(id: string): Promise<Area | null> {
    const row = await this.#database.prepare("SELECT * FROM areas WHERE id = ?").get(id);
  ...
  async createProjectWithStep(project: Project, step: Step): Promise<void> {
    ...
    await this.#database.transaction(async (database) => {
      const store = new LibsqlStore(database);
  ...
  async getProject(id: string): Promise<Project | null> {
    const row = await this.#database.prepare("SELECT * FROM projects WHERE id = ?").get(id);
  ...
  async openWeek(week: Week, closedAt: string): Promise<Week> {
    return this.#database.transaction(async (database) => {
      await database.prepare(`UPDATE weeks SET closed_at = ?, capacity_label = NULL,
        tag_id = NULL, reflection = NULL
        WHERE owner_id = ? AND starts_on < ? AND closed_at IS NULL`)
        .run(closedAt, week.ownerId, week.startsOn);
      await database.prepare(`INSERT INTO weeks (id, owner_id, starts_on)
        VALUES (?, ?, ?) ON CONFLICT(owner_id, starts_on) DO NOTHING`)
        .run(week.id, week.ownerId, week.startsOn);
      const row = await database.prepare("SELECT * FROM weeks WHERE owner_id = ? AND starts_on = ?")
  ...
  async getWeek(id: string, ownerId: string): Promise<Week | null> {
    const row = await this.#database.prepare("SELECT * FROM weeks WHERE id = ? AND owner_id = ?").get(id, ownerId);
  ...
  async getWeekStartingOn(ownerId: string, startsOn: string): Promise<Week | null> {
    const row = await this.#database.prepare("SELECT * FROM weeks WHERE owner_id = ? AND starts_on = ?")
      .get(ownerId, startsOn);
    return row === undefined ? null : toWeek(row as unknown as WeekRow);
  }

  async closeWeek(week: Week): Promise<boolean> {
    const result = await this.#database.prepare(`UPDATE weeks
  ...
  async listCommitments(weekId: string, ownerId: string): Promise<Commitment[]> {
    const rows = await this.#database.prepare(`SELECT * FROM commitments
  ...
  async writeCommitment(commitment: Commitment): Promise<boolean> {
    const result = await this.#database.prepare(`INSERT INTO commitments
  ...
  async spendReserve(entry: Entry, weekId: string): Promise<boolean> {
    const result = await this.#database.prepare(`INSERT INTO entries
  ...
  async readWeekEntries(ownerId: string, startsAt: string, endsAt: string): Promise<Entry[]> {
    const rows = await this.#database.prepare(`SELECT * FROM entries
  ...
  async getStep(id: string): Promise<Step | null> {
    const row = await this.#database.prepare("SELECT * FROM steps WHERE id = ?").get(id);
